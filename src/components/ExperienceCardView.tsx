...
export interface ExperienceCardPresentation {
  slug: string;
  title: string;
  shortTitle: string;
  hook: string;
...
interface ExperienceCardViewProps {
  card: ExperienceCardPresentation;
  variant?: ExperienceCardVariant;
  className?: string;
}

export const ExperienceCardView = ({ card, variant = "featured", className }: ExperienceCardViewProps) => {
        ...
        >
          ...
          <p
            className={cn(
              ...
              compact ? "line-clamp-2 min-h-10 text-xs leading-5" : "line-clamp-2 min-h-11 text-sm leading-[1.4rem]",
            )}
          >
            {card.hook}
          </p>
          ...
          <span
            className={cn("mt-auto inline-flex items-center gap-2 pt-5 text-sm font-black", compact && "text-xs")}
